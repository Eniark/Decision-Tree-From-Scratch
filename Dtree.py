...
class DecisionTree:
    def __init__(self, dataset: list[list], col_names:list[str], y_column_number:int, print_tree=True) -> None:
    ...
    def build_tree(self, node:Node) -> Node:
        ...
        X_l, y_l, X_r, y_r = node.split(question, self.col_names.index(question.column))
    ...
    def find_best_partition(self, node: Node) -> dict:
        ...
        best_criterion_to_split = sorted(ginis, key=lambda x: x['information_gain'], reverse=True)[0]
...
DTree = DecisionTree(data, col_names, 3)
